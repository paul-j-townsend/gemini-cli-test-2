...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Check if the new quiz format already exists
    try {
      await supabaseAdmin
        .from('quiz_questions')
      ...
      return res.status(200).json({ message: 'Quiz format migration already applied' });
    } catch (error: any) {
      if (!error.message.includes('column "learning_outcome" does not exist') && 
          !error.message.includes('relation "quiz_questions" does not exist')) {
        throw error;
      }
      // Columns don't exist, proceed with migration
    }

    console.log('Starting quiz format migration...');
...
(
    ...
    if (error) {
      ...
      await supabaseAdmin.rpc('exec_sql', { 
        sql: 'DROP TABLE IF EXISTS quiz_attempts CASCADE; DROP TABLE IF EXISTS quiz_questions CASCADE;' 
      });
    ...
    try {
      const sampleQuestions = [
        {
          ...
          learning_outcome: 'Analyze ethical dilemmas using established ethical frameworks relevant to veterinary nursing practice',
      ...
      if (insertError) {
        console.log('Sample data insert error (may already exist):', insertError);
      }
    } catch (insertErr) {
      console.log('Sample data insert failed:', insertErr);
    }

    return res.status(200).json({ 
      message: 'Quiz format migration completed successfully',
      details: 'Tables recreated with new format and sample data inserted'
    });
    ...
    return res.status(500).json({ 
      ...
      error: err.message,
      suggestion: 'You may need to run the migration manually in your Supabase dashboard using the SQL in sql/migrate_quiz_format.sql'
    });
  }
}