  ...
  return (
    <div className="space-y-4 mb-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="font-semibold text-emerald-900">Select the {hasMultipleCorrectAnswers ? 'correct answers' : 'best answer'}:</span>
        {hasMultipleCorrectAnswers && (
          <span className="bg-emerald-100 text-emerald-800 text-xs px-2 py-1 rounded-full font-medium">Multiple Selection</span>
        )}
      </div>
          ...
          return (
            <label 
              ...
              className={`flex items-start p-3 border-2 rounded-xl cursor-pointer transition-all duration-300 hover:shadow-md group ${
                isSelected && !showExplanation ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 
            ...
            >
              ...
              <div className="flex-shrink-0 mr-4">
                <div className={`w-10 h-10 flex items-center justify-center transition-all duration-300 font-bold text-sm ${
                  ...
                  isSelected && !showExplanation ? 'border-emerald-500 bg-emerald-500 text-white shadow-lg' :
                  showCorrect ? 'border-green-500 bg-green-500 text-white shadow-lg' :
                  isWrong ? 'border-red-500 bg-red-500 text-white shadow-lg' :
                  'border-emerald-300 bg-white text-emerald-600 group-hover:border-emerald-400 group-hover:bg-emerald-50 group-hover:text-emerald-600'
                }`}>
                  {!showExplanation ? (
                    letterOption
                  ) : (
              ...
              <div className="flex-grow">
                <div className={`text-lg leading-relaxed transition-all duration-200 ${
                  ...
                  isSelected && !showExplanation ? 'font-medium text-emerald-800' :
                  'text-emerald-800'
                }`}>
                  {answer.answer_text}
                </div>
